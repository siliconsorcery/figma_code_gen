...
function visit(
	node: DocumentNode | PageNode | SceneNode | null,
	nodeCount: number,
	...
	if ("children" in node) {
		node.children.forEach(
			function(node: PageNode | SceneNode | null) {
...
figma.ui.onmessage = msg => {
	...
	if (msg.type === 'outline') {
		let node: DocumentNode | PageNode | SceneNode | null = figma.root.findOne(node => node.type === 'FRAME' && node.name === 'Flutter')
		if (node === null) { node = figma.currentPage.selection[0] }
		if (node === null) { node = figma.root }
